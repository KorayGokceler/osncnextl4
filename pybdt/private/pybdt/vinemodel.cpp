...
VineModel::VineModel (const boost::python::list& feature_names,
                      const string& vine_feature,
...
VineModel::base_score (const Scoreable& e, bool use_purity) const
{
    ...
    for (size_t m (0); m < m_models.size (); ++m) {
        ...
        if (((bin_min <= value) and (value < bin_max))
            or ((value == bin_max) and (m == m_models.size () - 1))) {
...
{
    boost::python::class_<VineModel, boost::python::bases<Model> > (
        "VineModel", boost::python::init<boost::python::list,string,boost::python::list,boost::python::list,boost::python::list> ())
