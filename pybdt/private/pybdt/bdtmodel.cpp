...
BDTModel::BDTModel (const boost::python::list& feature_names,
...
BDTModel::event_variable_importance_py (
    const boost::python::list& vals, bool sep_weighted, bool tree_weighted) const
{
...
boost::shared_ptr<BDTModel>
BDTModel::get_subset_bdtmodel_list (boost::python::list dtmodel_indices) const
{
    vector<boost::shared_ptr<DTModel> > subset_dtmodels;
    vector<double> subset_alphas;
...
{
    if (use_purity) {
        // follow the SAMME.R algorithm from
        // J. Zhu, H. Zou, S. Rosset, T. Hastie.
        // "Multi-class AdaBoost", 2009.
        // https://web.stanford.edu/~hastie/Papers/samme.pdf
        // score is essentially the geometric mean of per-tree leaf node
        // signal purities divided by the sum of the geometric means of the
        // per-tree leaf node signal and background purities, and then
        // finally shifted to scale from -1 to +1
        // calculation in log space to deal with very small numbers

        double sum_log_sig_ratios (0);
        double sum_log_bg_ratios (0);
        const double EPS (Model::get_epsilon ());
        for (int m = 0; m < m_n_dtmodels; ++m) {
            const double s (m_dtmodels[m]->score (e, true));
            const double p (.5 * (s + 1));
            const double pm_sig (min (1 - EPS, max (EPS, p)));
            const double pm2_sig (pm_sig * pm_sig);
            const double pm_bg (1 - pm_sig);
            const double pm2_bg (pm_bg * pm_bg);
            const double denom (pm_sig - pm2_sig);
            sum_log_sig_ratios += log (pm2_sig / denom);
            sum_log_bg_ratios += log (pm2_bg / denom);
        }
        const double exp_sig (exp (sum_log_sig_ratios / (2 * m_n_dtmodels)));
        const double exp_bg (exp (sum_log_bg_ratios / (2 * m_n_dtmodels)));
        double norm (exp_sig + exp_bg);
        if (norm == 0 or not std::isfinite (norm)) {
            norm = 1;
        }
        const double p = (exp_sig / norm);
        const double score = 2 * p - 1;
        return min (1., max (-1., score));
    } else {
        // score is the weighted arithmetic mean of the per tree signal (+1)
        // or background (-1) votes
        double score (0.);
        for (int m = 0; m < m_n_dtmodels; ++m) {
            score += m_dtmodels[m]->score (e) * m_alphas[m]
                / m_max_response;
        }
        return min (1., max (-1., score));
    }
...
export_bdtmodel ()
{
    ...
    typedef const boost::python::list& clr;
