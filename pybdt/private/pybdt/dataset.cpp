...
DataSet::DataSet (const boost::python::dict& data, string subset)
: m_n_features (0), m_n_events (0), m_livetime (-1)
{
    // data should be a dict of (str, numpy.ndarray) items
    int n_input_cols (len (data));
    boost::python::list keys = data.keys ();
    keys.sort ();
    vector<vector<double> > cols;
    // get data into vectors
    for (int j = 0; j < n_input_cols; ++j) {
        string name = boost::python::extract<string> (keys[j]);
        if (name == "livetime") {
            m_livetime =  boost::python::extract<double> (data[name]);
            continue;
        }
        boost::python::object pycol = data[name];
        PyObject* C_numpy_col = pycol.ptr ();
        if (not PyArray_Check (C_numpy_col)) {
            // ignore non-array elements the user might be carrying around
            // in the initializer dict
            continue;
        }
...
DataSet::to_dict () const
{
    ...
    for (vector<string>::const_iterator i_name = m_names.begin ();
        ...
        out[*i_name] = boost::python::object (boost::python::handle<> (boost::python::borrowed (getitem (*i_name))));
    }
    return out;
}
...
export_dataset ()
{
    ...
    boost::python::class_<DataSet> (
        ...
        .def (boost::python::init<boost::python::dict,string> ())
