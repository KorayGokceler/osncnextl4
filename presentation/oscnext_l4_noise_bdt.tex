...
\begin{frame}{pass3 rates}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.58\textwidth}
      \footnotesize
      \tick~noise agrees -- this validates the vuvuzela weight unit

      \smallskip
      \cross~signal high -- our flux is a plain power law

      \smallskip
      \src{pass2 side is the note's, Table 13.  Nothing here reprocesses pass2.}
    \end{column}
  \end{columns}
  \note{The rate table is the single best test of the whole weighting chain.
  It already caught one bug: the weights were divided by the number of HDF5
  files instead of the number of L3 files, a factor of one hundred.  The
  signal excess is expected -- an invented power law instead of a real flux
  with oscillations.}
\end{frame}
...
\begin{frame}[c]{Noise BDT input variables (pass3)}
  \framesubtitle{definitions from Table 11 of the technical note}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.34\textwidth}
      ...
      \begin{tabular}{@{}>{\raggedright\arraybackslash}p{1.9cm}%
        ...
        \toprule
        \texttt{NchCleaned} & hit DOMs, cleaned series \\[3pt]
        \texttt{micro\_count} & DOMs in the densest time window \\[3pt]
        \texttt{iLineFit\_speed} & speed from the improved LineFit \\[3pt]
        \texttt{fill\_ratio} & filling of a sphere about the first HLC hit \\[3pt]
        \texttt{FullTime\-LengthRatio} & cleaned / uncleaned duration \\
        \bottomrule
      \end{tabular}
      ...
      \smallskip
      \texttt{FullTimeLengthRatio} separates the \emph{opposite} way to the
      expectation.
    \end{column}
  ...
  compact in time, not because noise is.}
\end{frame}
...
\begin{frame}[c]{Input correlations and feature importance}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \footnotesize
      \tick~the inputs are largely independent -- each one carries its own
      information, none is redundant
    \end{column}
    \begin{column}{0.49\textwidth}
      \footnotesize
      \cross~\texttt{fill\_ratio} carries most of the gain -- and its radius
      was never re-optimised for oscNext

      \smallskip
      \cross~\texttt{iLineFit\_speed} is nearly dead
    \end{column}
  \end{columns}
  \note{This is the argument for next step 2.  The variable the model leans on
  hardest is the one whose only free parameter is inherited from GRECO -- the
  original code says so itself.  Re-tuning it is the single highest-value knob
  we have.  Note the importance comes from the trained model, so strictly it
  is a result of the next two slides; it sits here because it belongs with the
