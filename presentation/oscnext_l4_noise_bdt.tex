...
\begin{frame}{The oscNext selection chain}
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{1.1cm}%
    ...
    \toprule
    \textbf{Stage} & \textbf{Objective, word for word} \\
    \midrule
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.56\textwidth}
      ...
      reconstruction, which the chain pays for only from L6 onward.
    \end{column}
  \end{columns}
...
\begin{frame}{What L4 does, and what had to be rebuilt}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \medskip
      Some of the inputs were \textbf{rewritten in pure Python}, from the
      reference document.
    \end{column}
    \begin{column}{0.50\textwidth}
      ...
      segment.  That comparison found bugs that \textbf{raise no error}.
    \end{column}
  \end{columns}
...
\begin{frame}{Noise BDT inputs, as the reference defines them}
  ...
  \begin{tabular}{@{}>{\ttfamily\raggedright\arraybackslash}p{4.8cm}%
    ...
    L4\_micro\_count.\allowbreak STW\_m3500p4000\_DTW200 &
    ...
    L4\_fill\_ratio.\allowbreak fill\_ratio\_from\_mean &
      ``Measure of the geometrical spread of the hits about some vertex
      \textcolor{bad}{(details here)}'' \\[4pt]
    IC2018\_LE\_L3\_Vars.\allowbreak FullTimeLengthRatio &
  ...
  \note{Everything in the Description column is the reference's own wording,
  word for word.  The NchCleaned row points at sec. 3.5.1, and that sentence
  sits beside it.
...
\begin{frame}[c]{Noise BDT inputs, signal vs.\ noise}
  \framesubtitle{Our own pass3 processing}
  \begin{center}
    \plot{4.png}
